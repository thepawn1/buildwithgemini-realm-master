...
def test_reasoning_engine_stream(server_fixture: subprocess.Popen[str]) -> None:
    ...
    has_content = any(
        ...
        and any(part.get("text") or "functionCall" in part or "executableCode" in part or "custom" in part for part in event["content"]["parts"])
        for event in events
    )
    assert has_content, "No meaningful content in reasoning_engine events"

...
class _SimpleUser(User):
    """User representation resolving a stable identity for cross-session memory."""
...
class _A2AServerCallContextBuilder(DefaultServerCallContextBuilder):
    ...
    and resolves a stable user identity for cross-session memory tracking.
    ...
    def build_user(self, request) -> User:
        user = super().build_user(request)
        if user.is_authenticated and user.user_name:
            return user
        user_id = (
            request.headers.get("x-user-id")
            or request.headers.get("user-id")
            or request.headers.get("x-goog-authenticated-user-email")
            or os.getenv("DEFAULT_USER_ID", "player_1")
        )
